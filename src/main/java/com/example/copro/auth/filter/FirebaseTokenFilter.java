...
public class FirebaseTokenFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
        ...
        validateAuthorizationHeader(header, response);
    ...
    private void validateAuthorizationHeader(String header, HttpServletResponse response) throws IOException {
        if (header == null || !header.startsWith("Bearer ")) {
            setUnauthorizedResponse(response, "INVALID_HEADER");
        }
    }

    private void authenticateWithFirebaseToken(String token) throws FirebaseAuthException {
        FirebaseToken firebaseToken = firebaseAuth.verifyIdToken(token);

        Member member = getMemberFromFirebaseToken(firebaseToken);
    ...
    private Member getMemberFromFirebaseToken(FirebaseToken firebaseToken) {
        try {
            return memberDetailService.updateByUsername(firebaseToken);
        } catch (MemberNotFoundException e) {
            return memberDetailService.create(firebaseToken, Role.ROLE_USER);
        }
    }
